<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="采购单号" prop="caigoudanhao">
            <el-input v-model="ruleForm.caigoudanhao" 
                placeholder="采购单号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.caigoudanhao" label="采购单号" prop="caigoudanhao">
              <el-input v-model="ruleForm.caigoudanhao" 
                placeholder="采购单号" readonly></el-input>
          </el-form-item>
        </div>
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `caigoujinhuodan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.caigoujinhuodanCrossAddOrUpdateFlag = false;
                  this.parent.search();
