<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="配件编号" prop="peijianbianhao">
          <el-input v-model="ruleForm.peijianbianhao" 
              placeholder="配件编号" clearable  :readonly="ro.peijianbianhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="配件编号" prop="peijianbianhao">
              <el-input v-model="ruleForm.peijianbianhao" 
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="修理单号" prop="xiulidanhao">
          <el-select v-model="ruleForm.xiulidanhao" placeholder="请选择修理单号">
            <el-option
                v-for="(item,index) in xiulidanhaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="修理单号" prop="xiulidanhao">
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/qichexiulidan/xiulidanhao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.xiulidanhaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `peijianshenqing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.peijianshenqingCrossAddOrUpdateFlag = false;
                  this.parent.search();
